...
import pydot
import matplotlib
matplotlib.use("QtAgg") #this has to be BEFORE import pyplot in order to run on MAC
...
class Trainer:
    ...
    def start_train_print_info(self, second_part = False):
        if not second_part:
            print(f"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Start Train {self.model_dictionary['model']} ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            print("~~~~~~~~~~~~~~~~~~~~~~~~~~ Data Shape ~~~~~~~~~~~~~~~~~~~~~~~~~")
    ...
    def train(self):
        self.X_train, self.X_test, self.y_train, self.y_test = self.split_dataset(self.X, self.y, test_size = (100 -self.model_dictionary["train_split"]) / 100)
    ...
    def train_random_forest(self):
        grid_search = self.find_best_params(RandomForestClassifier(), self.X_train)
        random_forest_model = RandomForestClassifier(**grid_search.best_params_)
        random_forest_model.fit(self.X_train, self.y_train)
        self.print_training_results(random_forest_model, self.X_train, self.X_test)
        self.active_model = random_forest_model
        self.max_image_index = self.active_model.get_params()["n_estimators"]


    def train_decision_tree(self):
        grid_search = self.find_best_params(DecisionTreeClassifier(), self.X_train)
        decision_tree_model = DecisionTreeClassifier(**grid_search.best_params_)
        decision_tree_model.fit(self.X_train, self.y_train)
        self.print_training_results(decision_tree_model, self.X_train ,self.X_test)
    ...
    def train_generalized_lvq(self):
        ...
        glvq_model = GLVQ(**grid_search.best_params_)  #GrlvqModel(**grid_search.best_params_)
    ...
    def find_best_params(self, model, X_train):
        params = self.convert_from_string_in_dictionary(self.model_dictionary["parameters"])
        grid_search = GridSearchCV(model, param_grid=params, scoring='f1_weighted', cv=5)
    ...
    def get_lvq_images(self, model=None):
        ...
        if len(self.most_important_features_names) > 3:
            ...
            self.max_image_index = 2 ** len(self.most_important_features_names) - 1
        ...
        self.features_to_plot.insert(len(self.features_to_plot.columns), "Label", predicted_labels)
    ...
    def plot_images(self, popup = False):
        match self.max_image_index:
            ...
            case 7: # plotting 3 featires (A : A, B : B, C : C, A : B, A : C, B : C, A : B : C)
                match self.active_image_index:
                    case 0:
                        self.plot_single_plot(0,0)
                    ...
                    case 6:
                        ...
                        ax = fig.add_subplot(projection='3d', facecolor=self.image_background_color)
    ...
    def get_statistics(self):
        ...
        precision = precision_score(self.y_test, labels_prediction, average='weighted')
        recall = recall_score(self.y_test, labels_prediction, average='weighted')
        return {
    ...
    def perform_auto_ml(self, train_split):
        with open(Model().get_all_ml_file_path(), 'r') as file: # Read content from all_ml_dictionary
